...
def time_warp(spectrogram, time_warp_w):
    ...
    if time_warp_w > 0:
        if 2 * time_warp_w < num_frames:
            import cv2

            w0 = np.random.randint(time_warp_w, num_frames - time_warp_w)
            w = np.random.randint(-time_warp_w + 1, time_warp_w)
            left, right = spectrogram[:, :w0], spectrogram[:, w0:]
            left = cv2.resize(
                left, dsize=(w0 + w, num_freqs), interpolation=cv2.INTER_LINEAR
            )
            right = cv2.resize(
                right,
                dsize=(num_frames - w0 - w, num_freqs),
                interpolation=cv2.INTER_LINEAR,
            )
            spectrogram = np.concatenate((left, right), axis=1)
